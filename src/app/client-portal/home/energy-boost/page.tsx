...
import { EnergyService } from '@/lib/client-portal/energy-service'
import { Message, ConversationStep, Intervention } from '@/lib/client-portal/craving-types'
import { Option } from '@/lib/client-portal/energy-conversation'
import { toast } from '@/components/ui/use-toast'

export default function EnergyBoostPage() {
  ...
  const energyServiceRef = useRef<EnergyService | null>(null)
  ...
  const addMessage = async (message: Message) => {
    if (!energyServiceRef.current) return;
    try {
      await energyServiceRef.current.saveMessage(message);
  ...
  // Initialize energy service and chat
  useEffect(() => {
    let mounted = true;
    ...
    const initChat = async () => {
      ...
      try {
        // Create and initialize energy service instance
        if (!energyServiceRef.current) {
          energyServiceRef.current = new EnergyService()
        }

        // Initialize service and wait for client ID
        if (energyServiceRef.current && mounted) {
          ...
          const initialized = await energyServiceRef.current.initialize()
          ...
          const sessionInfo = await energyServiceRef.current.getSessionInfo()
          ...
          if (mounted) {
            ...
            if (sessionInfo.coach) {
              ...
              if (storedClientId && energyServiceRef.current) {
                try {
                  const clientDetails = await energyServiceRef.current.fetchClientDetails(storedClientId);
                  if (clientDetails && clientDetails.coach_id) {
                    const coachDetails = await energyServiceRef.current.getCoachDetails(clientDetails.coach_id);
                    if (coachDetails) {
                      setCoach({
            ...
            incidentId = await energyServiceRef.current.createMovementIncident();
            if (!incidentId) {
              toast.error('Unable to start your energy boost session. Please try refreshing the page or contact your coach.');
              return;
            }
            setIncidentCreated(true);
            
            // Double-check that the incident ID is set in the service
            if (energyServiceRef.current.getIncidentId() !== incidentId) {
              return;
            }

            // Start directly with blocker identification message (skip redundant welcome)
            const blockerSelectionRes = await energyServiceRef.current.getBlockerIdentificationMessage(firstName);
            
            // Save blocker selection message directly using the incident ID
            if (energyServiceRef.current) {
              await energyServiceRef.current.saveMessage(blockerSelectionRes.response);
            }
            setMessages(prev => [...prev, blockerSelectionRes.response]);
            
            // Update state for blocker selection
            setCurrentStep(blockerSelectionRes.nextStep);
            setOptionChoices(blockerSelectionRes.options || []);
          }
        }
      } catch (error) {
        console.error('❌ Chat initialization failed:', error);
        setInitializationError('Unable to initialize your energy boost session. Please refresh the page or check your internet connection.');
        toast.error('Unable to initialize your energy boost session. Please refresh the page or check your internet connection.');
        setIsInitializing(false);
      }
    }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await energyServiceRef.current.processUserInput({
        input: messageText,
        currentStep,
        clientName,
        chosenIntervention,
  ...
  const handleIntensitySelect = async (level: number) => {
    ...
    try {
      ...
      toast.error('Unable to record your energy level. Please try again.');
      setIsLoading(false);
    }
  };
                  ...
                  currentStep === ConversationStep.CLOSE ? (
                    <div
                      ...
                      className={`w-8 h-8 rounded-full flex items-center justify-center cursor-default
                        ${level <= 3 ? 'bg-red-50 text-red-400' : 
                    ...
                    <button
                      ...
                      className={`w-8 h-8 rounded-full flex items-center justify-center
                        ${level <= 3 ? 'bg-red-100 text-red-700 hover:bg-red-200' : 
      ...
      <div className="flex flex-col h-[calc(100vh-4rem)]">
        <div className="bg-orange-500 text-white p-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="h-12 w-12 rounded-full bg-white p-0.5">
              <div className="h-full w-full rounded-full bg-orange-100 animate-pulse" />
            </div>
            <div>
          ...
          <button 
            ...
            className="text-white text-sm px-3 py-1 rounded hover:bg-orange-600"
          >
            Close
          </button>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <div className="text-center">
            <div className="inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm shadow rounded-md text-orange-500 bg-white">
              ...
              Setting up your energy boost session...
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
          ...
          <div className="h-12 w-12 rounded-full bg-white p-0.5 overflow-hidden">
            {coach.avatarUrl ? (
              <div className="relative h-full w-full">
                ...
                />
                {avatarLoading && (
                  <div className="absolute inset-0 rounded-full bg-orange-100 animate-pulse flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-4">
        {messages.map((message, index) => (
          ...
          >
            {message.sender === 'coach' && (
              <div className="h-8 w-8 rounded-full bg-orange-100 mr-2 overflow-hidden flex-shrink-0">
                {coach.avatarUrl ? (
                  <div className="relative h-full w-full">
                    <Image 
                      src={coach.avatarUrl} 
                  ...
                  <div className="h-full w-full rounded-full bg-orange-300 flex items-center justify-center text-white text-sm font-semibold">
                    {coach.name ? coach.name.charAt(0).toUpperCase() : 'C'}
                  </div>
                )}
              </div>
            ...
            >
              ...
              <div className={`text-xs mt-1 ${message.sender === 'client' ? 'text-orange-100' : 'text-gray-500'}`}>
      ...
      <div className="p-4 border-t border-gray-200 bg-white">
        <div className="flex items-center space-x-2">
          <input
            ...
            className="flex-1 border border-gray-300 rounded-full px-4 py-2 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:bg-gray-100 disabled:text-gray-500"
